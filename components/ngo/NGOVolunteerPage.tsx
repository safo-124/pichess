...
import type { NGOVolunteerContent } from "@/lib/ngo-content";

export default function VolunteerPage({ content }: { content: NGOVolunteerContent }) {
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <AnimatedSection delay={0.05} className="grid grid-cols-3 gap-3 mb-10">
          {content.benefits.map((b, i) => (
            <div key={b.title} className="rounded-xl bg-[#d4ede3] border border-[#2e7d5b]/15 p-4 text-center">
              <div className="text-2xl mb-1">{b.icon}</div>
              <p className="text-xs font-semibold text-zinc-700">{b.title}</p>
            </div>
          ))}
        </AnimatedSection>
